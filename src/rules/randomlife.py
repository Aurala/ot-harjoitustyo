    ...
    def calculate(self, universe: Universe):
        ...
        if random.randint(1, 100) <= self.rate:
            for _ in range(0, random.randint(10)):
                universe.add_cell(random.randint(universe.width), random.randint(universe.height))
                universe.erase_cell(random.randint(universe.width), random.randint(universe.height))
